'''
This script creates a test split of maxNum instances per class (min. minNum if class is rare) using the image filepath and rating filepath.
'''
...
img_path = '../project/all_males_combined'
rating_path = './ratings_g_combined.txt'
save_path_test = '../project/eval_data/all_males_combined_test_s'
save_path_train = '../project/eval_data/all_males_combined_train_s'
maxNum = 7
minNum = 1
...
text_file = open(save_path_test + "/test_ratings.txt", "w")
...
for r,sub in enumerate(selected_img):
    for img in sub:
        ...
        #move selected_img to separate folder & create corresponding test_ratings.txt
        copyfile(img_path + "/" + img,save_path_test + "/" + str(orig_ratings[idx]) + "_" + img) #copy rating into filename to ensure correct order
